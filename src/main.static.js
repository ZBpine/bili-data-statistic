...
import { DEFAULT_THEME, normalizeThemeColor, normalizeThemeMode } from './utils/theme';
...
const hasHttpBridge = () => typeof globalThis.__BDS_HTTP_REQUEST__ === 'function';

if (!BiliDataManager) throw new Error('BiliDataManager 未加载');
const getThemeLoggerColor = () => {
  const mode = normalizeThemeMode(storage.get('theme.mode', DEFAULT_THEME.mode));
...
let currentBDM = createBDM(hasHttpBridge() ? globalThis.__BDS_HTTP_REQUEST__ : null);
const BDM = new Proxy({}, {
  get(_, prop) {
